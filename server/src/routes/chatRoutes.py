...
async def send_message(
    ...
    try:
        ...
        if strategy == 'basic':
            chunks = vector_search(message_content, document_ids, settings) 
            print(f"✅ Retrieved {len(chunks)} relevant chunks from vector search") 

        elif strategy == 'hybrid':
            print("📈 Executing: Hybrid Search (Vector + Keyword)") 
            chunks = hybrid_search(message_content, document_ids, settings)
            ...
            queries = generate_query_variations(message_content, settings['number_of_queries'])
        ...
        #ai_response ="This is a test reponse from the AI assistant"
